...
export function create_seaport_buynow_bundle_transaction(item: CovalentItem): Transaction[] {
  const transactions: Transaction[] = [];
  const purchaser = item.log_events[1]?.decoded?.params[1]?.value.toString().toLowerCase();
  const seller = item.log_events[1]?.decoded?.params[0]?.value.toString().toLowerCase();
